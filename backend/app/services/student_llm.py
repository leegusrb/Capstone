...
5. 막연한 질문은 하지 마세요.
   금지 패턴: "자세히 설명해주세요", "다른 부분은 어떤 것들이 있나요?", "더 설명해주세요", "어떤 것들이 있나요?"
   대신 특정 측면(역할·원리·차이·과정·예시 중 하나)을 명시해서 질문하세요.
   나쁜 예: "CNN 구조에 대해 다른 부분은 어떤 것들이 있나요?"
   좋은 예: "CNN에서 풀링 레이어는 어떤 역할을 하나요?"
...
def _compute_direction_block(
    ...
    if partial_nodes:
        return "\n".join([
            "▶ 부분 이해 개념 보완 → 특정 측면을 명시한 질문",
            f"  대상 개념: [{', '.join(partial_nodes)}]",
            "  선생님이 방금 설명한 내용에서 자연스럽게 이어지는 특정 측면(역할·원리·예시·작동 방식 중 하나)을 명시해 질문하세요.",
            "  ⚠ '다른 부분은 어떤 것들이 있나요?', '더 설명해주세요' 같은 열린 질문 금지.",
        ])
    return "\n".join([
        "▶ 전반적 이해 양호 → 새 내용 유도",
