...
class EvaluatorResult:
    ...
    specificity_checklist: dict = field(default_factory=dict)
...
4. 오개념이 있으면 기록합니다.
5. 이번 설명의 구체성(specificity)을 4가지 기준으로 판정합니다.

※ 루브릭 점수(concept / accuracy / logic)는 시스템이 User KG 누적 상태로부터 자동 계산합니다.
   당신이 직접 점수를 매기지 마세요. 구체성(specificity)만 아래 기준으로 true/false 판정하세요.
...
━━━ 구체성(specificity) 판정 ━━━
이번 턴 사용자 설명 전체를 기준으로 아래 4가지를 각각 true/false로 판정하세요.
KG 체크리스트와 무관하게, 설명 텍스트 자체의 구체성만 평가합니다.

- example_present   : 예시·시나리오·실제 사례가 한 개 이상 등장했는가?
- concrete_terms    : 수치·단위·고유명사(프로토콜명, 알고리즘명 등)가 포함됐는가?
- sentence_explained: 개념 이름 나열에 그치지 않고 의미·원리·동작을 문장으로 서술한 부분이 있는가?
- context_applied   : 추상적 설명을 넘어 적용 상황이나 활용 맥락을 언급했는가?
...
{{
  ...
  "specificity_checklist": {{
    ...
    "concrete_terms": true|false,
    "sentence_explained": true|false,
...
def compute_rubric_scores(
    ...
    specificity_checklist: dict,
) -> RubricScores:
    """
    User KG 누적 상태와 specificity_checklist를 기반으로 루브릭 4개 영역 점수를 계산한다.

    점수 기준:
      concept    : (confirmed + partial 노드) / Reference 전체 노드
                   ≥80%→3 / ≥50%→2 / ≥20%→1 / <20%→0
      accuracy   : confirmed 노드 수 / Reference 전체 노드 수 (오개념 패널티 적용)
                   ratio≥0.8 & misc=0→3 / ratio≥0.5 & misc≤1→2 / ratio≥0.2 & misc≤3→1 / else→0
      logic      : (confirmed + partial 엣지) / Reference 전체 엣지
                   ≥60%→3 / ≥30%→2 / ≥10%→1 / <10%→0
      specificity: specificity_checklist 4항목 중 true 개수
                   ≥3→3 / 2→2 / 1→1 / 0→0
    """
    # ── concept ──
    ...
    mentioned_count = len(confirmed) + len(partial)
    node_ratio = mentioned_count / total_nodes if total_nodes > 0 else 0.0
    concept = 3 if node_ratio >= 0.8 else 2 if node_ratio >= 0.5 else 1 if node_ratio >= 0.2 else 0

    # ── accuracy ──
    confirmed_ratio = len(confirmed) / total_nodes if total_nodes > 0 else 0.0
    misc_count = len([
        n for n, attrs in user_kg.nodes(data=True)
        if attrs.get("status") == NodeStatus.MISCONCEPTION and not str(n).startswith("__")
    ])
    if "__misconceptions__" in user_kg:
        misc_count = max(misc_count, len(user_kg.nodes["__misconceptions__"].get("items", [])))

    if confirmed_ratio >= 0.8 and misc_count == 0:
        accuracy = 3
    elif confirmed_ratio >= 0.5 and misc_count <= 1:
        accuracy = 2
    elif confirmed_ratio >= 0.2 and misc_count <= 3:
        accuracy = 1
    else:
        accuracy = 0

    # ── logic ──
    total_edges = reference_kg.number_of_edges()
    mentioned_edges = (
        len(get_edges_by_status(user_kg, EdgeStatus.CONFIRMED)) +
        len(get_edges_by_status(user_kg, EdgeStatus.PARTIAL))
    )
    edge_ratio = mentioned_edges / total_edges if total_edges > 0 else 0.0
    logic = 3 if edge_ratio >= 0.6 else 2 if edge_ratio >= 0.3 else 1 if edge_ratio >= 0.1 else 0

    # ── specificity ──
    true_count = sum(1 for v in specificity_checklist.values() if v is True)
    specificity = 3 if true_count >= 3 else 2 if true_count == 2 else 1 if true_count == 1 else 0
...
def evaluate_explanation(
    ...
    specificity_checklist = data.get("specificity_checklist", {})
    ...
    return EvaluatorResult(
        ...
        specificity_checklist=specificity_checklist,
    )
