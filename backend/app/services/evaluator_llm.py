...
4. 오개념이 있으면 기록합니다.
5. 각 노드에 대해 사용자가 얼마나 확신 있게 설명했는지 confidence_level을 판정합니다.

※ 루브릭 점수(concept / accuracy / logic / specificity) 4가지 모두 시스템이 User KG 누적 상태로부터 자동 계산합니다.
   당신이 직접 점수를 매기지 마세요. 노드별 confidence_level만 아래 기준으로 판정하세요.
...
━━━ 노드별 confidence_level 판정 ━━━

각 언급 노드에 대해 사용자가 해당 개념을 얼마나 확신 있게 설명했는지를
"high" / "medium" / "low" 중 하나로 판정하세요.
해당 노드를 설명하는 부분만 기준으로 판정하고, 문장 전체를 기준으로 삼지 마세요.

▸ high — 확신 있는 설명
  조건 (아래 중 하나 이상 해당):
  - 노드와 관련된 구체적 수치·고유명사·프로토콜명·알고리즘명 포함
    예) "3-way handshake", "ACK 패킷", "64비트", "FIFO 방식"
  - 인과관계나 동작 원리를 능동적 문장으로 서술
    예) "A가 발생하면 B가 트리거되어 C 상태가 된다"
  - 예시나 구체적 시나리오를 직접 제시하며 설명
    예) "예를 들어 클라이언트가 SYN을 보내면..."

▸ medium — 부분적 확신
  조건 (아래 중 하나 이상 해당):
  - 핵심 내용은 서술했지만 보조 설명에 불확실 표현 혼재
    예) "TCP는 연결 지향적인데, 정확히 어떻게 연결하는지는 잘 모르겠어요"
  - 개념의 일부만 서술하고 나머지는 생략
  - "~인 것 같아요"가 일부 있지만 전반적으로 내용은 갖춰진 경우

▸ low — 불확실한 설명
  조건 (아래 중 하나 이상 해당):
  - 추측성 표현이 핵심 서술을 감싸는 경우: "아마", "~것 같은데", "맞는지 모르겠지만"
  - 확인 요청: "이게 맞나요?", "~아닌가요?"
  - 이름만 언급하고 의미 서술 없음 (유형 A 노드는 자동으로 low)
...
def compute_rubric_scores(
    ...
    점수 기준:
      concept    : ((1.0 * confirmed 노드 수) + (0.5 * partial 노드 수)) / Reference 전체 노드 수
                   ratio≥0.8→3 / ratio≥0.5→2 / ratio≥0.2→1 / ratio<0.2→0
      accuracy   : ((1.0 * confirmed 노드 수) + (0.5 * partial 노드 수)) / (confirmed + partial + misconception 노드 수) (오개념 패널티 적용)
                   ratio≥0.8 & misconception=0→3 / ratio≥0.5 & misconception≤1→2 / ratio≥0.2 & misconception≤3→1 / else→0
      logic      : ((1.0 * confirmed 엣지 수) + (0.5 * partial 엣지 수)) / (confirmed + partial + misconception 엣지 수)
                   ratio≥0.8→3 / ratio≥0.5→2 / ratio≥0.2→1 / ratio<0.2→0
    ...
    if mentioned_node_count == 0:
        ...
        if accuracy_score >= 0.8 and misc_count == 0:
            accuracy = 3
        elif accuracy_score >= 0.5 and misc_count <= 1:
            accuracy = 2
    ...
    if mentioned_edge_count == 0:
        ...
        logic = 3 if logic_score >= 0.8 else 2 if logic_score >= 0.5 else 1 if logic_score >= 0.2 else 0
    ...
    if not all_mentioned:
        ...
        specificity = 3 if ratio >= 0.8 else 2 if ratio >= 0.5 else 1 if ratio >= 0.2 else 0
