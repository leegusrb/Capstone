...
[3단계] 체크리스트 met 판정 (유형 B 노드에만 적용)
  각 체크리스트 항목에 대해 아래 세 단계를 순서대로 적용하세요.

  [1차 판정] ← 반드시 먼저 수행
    사용자 설명 전체를 처음부터 끝까지 검색해 해당 체크리스트 항목의 핵심 내용을
    직접 서술한 문장을 찾으세요.
    ▸ 긴 설명에서 관련 내용이 여러 단락·문장에 걸쳐 있어도 모두 합산합니다.
    ▸ 표현이 달라도 되지만, 해당 항목의 핵심 사실을 사용자가 직접 서술해야 합니다.
      (주제만 같거나 개념 이름만 등장한 문장은 근거로 인정하지 않습니다.)
    ▸ 핵심 내용을 직접 서술한 문장을 찾지 못한 경우 met=false로 즉시 확정하고 이후 단계를 건너뜁니다.

  [2차 판정] source_quote 직접 대조
    추출한 근거 문장이 source_quote의 내용을 그대로 포함하거나 매우 유사한 표현으로 서술했는가?
    → 해당하면 met=true 확정

  [3차 판정] 의미적 동등성 확인 (2차에서 false인 경우에만 적용)
    추출한 근거 문장이 source_quote와 표현은 다르지만 동일한 개념·원리·사실을 전달하고 있는가?
    → 해당하면 met=true
    → 해당하지 않으면 met=false

    ⚠ 아래 경우는 반드시 met=false입니다:
    - 사용자가 해당 개념의 이름만 언급하고 내용을 서술하지 않은 경우
    - 학습자료(RAG)에는 해당 내용이 있지만 사용자 설명에는 없는 경우
    - 사용자 설명에서 주제만 같고 해당 사실을 직접 서술하지 않은 경우
    RAG 검색 결과는 판정 기준일 뿐, 사용자가 말한 내용이 아닙니다.
...
위의 RAG 검색 결과나 Reference KG 정보는 평가 기준 참고자료이며, 사용자가 말한 내용이 아닙니다.
체크리스트 met 판정은 반드시 아래 텍스트에 근거해야 합니다.
